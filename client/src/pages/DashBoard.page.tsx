...
const DashboardPage = () => {
  ...
  const handleTemChoose = (templateId: string) => {
    if (!data) return;
    const payLoad = {
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
              <Suspense fallback={<CircularProgress size="3rem" />}>
                {!data?.user?.cvTemplateId ? (
                  <Typography color="text.secondary">No CVs created yet.</Typography>
                ) : (
                  <HoverCard
                    key={data?.user?.cvTemplateId}
                    img={
                      templateJson[data?.user?.cvTemplateId as keyof typeof templateJson]
                        .img
                    }
                    hoverText={HoverText.EDIT}
                    onClick={() => EditHandler(data?.user?.cvTemplateId)}
                  />
                )}
              </Suspense>
            </Box>
              ...
              <Suspense fallback={<CircularProgress size="3rem" />}>
                {templateData?.templates.map((id) => (
                  <HoverCard
                    ...
                    hoverText={
                      data?.user?.cvTemplateId === id ? HoverText.EDIT : HoverText.NEW
                    }
                    onClick={
                      data?.user?.cvTemplateId === id
                        ? () => EditHandler(id)
                        : () => handleTemChoose(id)
                    }
                    disable={data?.user?.cvTemplateId === id}
                  />
                ))}
              </Suspense>
            </Box>
